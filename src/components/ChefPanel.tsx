...
import { CATEGORIES, MENU_ITEMS_DATA } from "../constants";
import {
  Card,
  CardContent,
  CardHeader,
...
export function ChefPanel() {
  ...
  const handleApplyCategoryDiscount = (categoryId: string) => {
    const discount = prompt(
      "Enter discount percentage for this category (0-100):",
    );
    if (discount !== null) {
      const d = parseInt(discount);
      if (!isNaN(d) && d >= 0 && d <= 100) {
        ...
        toast.success(
          `Applied ${d}% discount to ${categories.find((c) => c.id === categoryId)?.name}`,
        );
      } else {
        toast.error("Invalid discount value");
      }
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage your menu categories, items, and promotions.
          </p>
        </div>
          ...
          <Card className="border-border/50 bg-primary/5 border-primary/20">
            ...
            <CardContent className="p-4 pt-0 space-y-3">
              ...
              <p className="text-[10px] text-muted-foreground italic">
                Discounts are applied instantly to the digital menu and
                checkout.
              </p>
            </CardContent>
          </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-6">
              ...
              <motion.div layout onClick={() => openItemModal()}>
                <Card className="border-2 border-dashed border-border/50 hover:border-primary/50 hover:bg-primary/5 transition-all duration-300 h-full flex flex-col items-center justify-center p-8 cursor-pointer group min-h-[280px]">
                  ...
                  <h3 className="font-bold text-muted-foreground group-hover:text-primary">
                    Add New Item
                  </h3>
